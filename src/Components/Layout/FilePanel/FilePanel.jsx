...
function FilePanel(props) {
  ...
  const clearTabPane = async () => {
    try {
      await deleteFileActionStatus(projectCode, sessionId);
      dispatch(setUploadListCreator([]));
      dispatch(setDownloadListCreator([]));
    } catch (e) {
      console.error(e);
      message.error('Clear tab pane failed');
    }
  };
    ...
    if (item.status === 'waiting' && tabName === 'progress') {
      ...
      const nameZone = (item) => {
        if (!item.payload) {
          ...
          if (item.payload.zone === 'Greenroom') {
            return 'Green Room';
          }
          return item.payload.zone;
        }
      };
      return (
        <span>
