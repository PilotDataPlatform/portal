...
describe('Collaborator project banner  ', () => {
  ...
  it('perpare user is collaborator', async () => {
    ...
    const contributor = await page.$x(
      "//tr[@data-row-key='testautomation']//td[text()='Project Collaborator']",
    );
    if (contributor.length > 0) {
      ...
      await page.waitForXPath(
        "//li[contains(@class,'ant-dropdown-menu-item') and text()='Project Collaborator']",
      );
    ...
    it('3.1.5 The user role in the setting page should displayed as Project Collaborator', async () => {
      const role = await page.waitForXPath(
        "//label[@title='Project Administrator' and text()='Project Collaborator']",
      );
    ...
    it('3.1.1,4 Project Contributor should be able to see Project Information (Title, Code, Description, Tags)', async () => {
