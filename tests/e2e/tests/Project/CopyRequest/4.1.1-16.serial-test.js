...
describe('CopyRequest', () => {
  ...
  it('prepare files', async () => {
    ...
    if (!fs.existsSync(`${process.cwd()}/tests/uploads/test/${fileName1}`)) {
      await createDummyFile('Test Files', fileName1, '10kb');
    }
    await page.waitForTimeout(3000);
    await uploadFile(page, 'Test Files', fileName1);
    await page.waitForTimeout(3000);
    await uploadFile(page, 'Test Files', fileName2);
    await page.goto(`${baseUrl}project/${projectCode}/data`);
  });
  ...
  //   }
  //   await page.goto(`${baseUrl}project/${projectCode}/data`);
  //   prepareActions(page, false, true);
  //   await submitCopyRequest(page);
